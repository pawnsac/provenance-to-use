...
static void unseize_task_and_threads(const struct pstree_item *item, int st)
{
	...
	for (i = 1; i < item->nr_threads; i++)
		if (ptrace(PTRACE_SYSCALL, item->threads[i].real, NULL, NULL))
			pr_perror("Unable to restart from %d", item->threads[i].real);
}
...
static int collect_threads(struct pstree_item *item)
{
	...
	for (i = 0; i < nr_threads; i++) {
		...
		// parse_pid_status(pid, &t_creds.s, NULL);
		// ret = TASK_STOPPED;
...
int collect_pstree(void)
{
	...
	if (ret < 0)
		goto err; */
	parse_pid_status(pid, &creds.s, NULL);
	ret = TASK_STOPPED;
